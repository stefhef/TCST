...
export const ChatMessagesActionCreators = {
	...
	addChatMessage: (payload: IChatMessage): AddChatMessageAction => ({
		type: ChatMessageActionEnum.ADD_CHAT_MESSAGE,
		payload,
	}),
	setChatMessages: (payload: IChatMessage[]): SetChatMessagesAction => ({
		type: ChatMessageActionEnum.SET_CHAT_MESSAGES,
		payload,
	}),
	setErrorChatMessages: (payload: string): SetErrorChatMessagesAction => ({
	...
	postChatMessage:
		...
		async (dispatch: AppDispatch) => {
			const newMessage = await ChatMessageService.postChatMessage(
				groupId,
				courseId,
				taskId,
				userId,
