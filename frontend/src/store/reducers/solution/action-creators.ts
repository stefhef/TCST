...
export const SolutionActionCreators = {
	clearSolution: (): ClearSolutionAction => ({
		type: SolutionActionEnum.CLEAR_SOLUTION,
	}),
	setSolution: (solution: ISolution | null): SetSolutionAction => ({
		type: SolutionActionEnum.SET_SOLUTION,
		payload: solution,
	}),
	setIsLoadingSolution: (payload: boolean): SetIsLoadingSolutionAction => ({
		type: SolutionActionEnum.SET_SOLUTION_IS_LOADING,
		payload,
	}),
	setIsChangedSolution: (payload: boolean): SetIsChangedAction => ({
		type: SolutionActionEnum.SET_SOLUTION_IS_CHANGED,
		payload,
	}),
	setErrorSolution: (payload: string): SetErrorSolutionAction => ({
		type: SolutionActionEnum.SET_SOLUTION_ERROR,
		payload,
	}),
	setCodeSolution: (payload: string): SetCodeSolutionAction => ({
		type: SolutionActionEnum.SET_SOLUTION_CODE,
		payload,
	}),
	setScoreSolution: (payload: number): SetScoreSolutionAction => ({
	...
	fetchBestSolution:
		...
		async (dispatch: AppDispatch) => {
			...
			if (user_id) {
				const response = await SolutionService.getBestSolution(
					groupId,
					courseId,
					taskId,
					user_id
