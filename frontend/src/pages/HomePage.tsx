...
const HomePage: FunctionComponent = () => {
	...
	useEffect(() => {
		if (isAuth && data) {
			const courses = data.get_all_courses.courses
				.map((courses: IHomePageData): ICoursePreview => {
					return {
				...
				.flat()
				.sort(
					(a: ICoursePreview, b: ICoursePreview) => a.courseId - b.courseId
				);
	...
	if (coursePreviews.length !== 0) {
		const previews = coursePreviews.map((v, index) => (
			<CoursePreview {...v} key={index} />
		));
		return isLargerThan768 ? (
			<Layout
				headerChildren={<Heading>Курсы</Heading>}
				mainChildren={
	...
	return (
		<Layout
			headerChildren={<Heading>Курсы</Heading>}
			mainChildren={<Heading>Нет доступных курсов</Heading>}
		/>
	);
};
