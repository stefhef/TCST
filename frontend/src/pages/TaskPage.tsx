...
export default function TaskPage() {
	...
	const { setSolution, setSelectedUser, clearSolution, clearSelectedUser } =
		useActions();
	const { current_solution, isChanged: solutionIsChanged } = useTypedSelector(
	...
	const [asideChildWidthStudentsList, setAsideChildWidthStudentsList] =
	...
	useEffect(() => {
		if (groupRole === IGroupRole.STUDENT) {
			...
			SolutionService.getBestSolution(groupId!, courseId!, taskId!).then(
				(solution) => {
			...
			groupRole === IGroupRole.OWNER
		) {
			setAsideChildWidthStudentsList(<TaskStudentsList />);
		}
		setIsLoading(false);
	}, [groupRole]);
	...
	useEffect(() => {
		if (selectedUser)
			SolutionService.getAllTaskSolutionsByUserId(
				groupId!,
				courseId!,
				taskId!,
				selectedUser.id
	...
	return (
		<Dropzone onDrop={(acceptedFiles) => sendFile(acceptedFiles[0])}>
					...
					<Layout
						...
						headerChildren={
							<>
								...
								<>
									<Button onClick={() => onOpen()} key={'lg'} mb={4}>
										{'Условие задачи'}
									</Button>
									<Drawer onClose={onClose} isOpen={isOpen} size={'lg'}>
										...
										<DrawerContent borderRadius="3px">
											<DrawerCloseButton />
											<DrawerHeader fontSize="4xl" borderBottomWidth="1px">
												{task?.name}
											</DrawerHeader>
											<DrawerBody>
												<Text style={{ whiteSpace: 'pre-wrap' }} fontSize="lg">
													{task?.description}
												</Text>
												<br />
						...
						mainChildren={
							isDragActive && groupRole === IGroupRole.STUDENT ? (
								<Box
										...
										<>
											{groupRole === IGroupRole.STUDENT && (
												<div>
													<Button
														onClick={async () => {
															await SolutionService.postSolutionCode(
																groupId!,
																courseId!,
																lessonId!,
																taskId!,
													...
													>
														Отправить решение
														<Icon as={BiSend} textAlign="center" w="6" h="6" />
													</Button>
						...
						asideChildren={asideChildWidthStudentsList}
					/>
				</div>
			)}
		</Dropzone>
