...
export function Chat() {
	const { chatMessages, isLoadingChatMessages } = useTypedSelector(
		(state) => state.chatMessages
	);
	const { fetchChatMessages, clearChatMessages } = useActions();
	const { groupId, courseId, taskId } = useParams();
	const { selectedUser, isLoading: isLoadingSelectedUser } = useTypedSelector(
		(state) => state.selectedUser
	);
	const messagesEndRef = useRef<null | HTMLDivElement>(null);
	const { users, usersIsLoading } = useTypedSelector(
	...
	useEffect(() => {
		...
		const neededUsersIds = chatMessages
			.map((message) => message.from_id)
			.filter((x) => !loadedUsersIds.includes(x));
		const neededUsersIdWithoutRepeats = neededUsersIds.filter(function (
			elem,
			index,
			self
		) {
