	...
	return (
		<Formik
			...
			onSubmit={async (values, actions) => {
				...
				if (values.score > maxScore) sendScore = maxScore;
				else if (values.score < 0) sendScore = 0;
				const updateSolution = await SolutionService.postSolutionChangeScore(
					current_solution!.id,
					false,
					sendScore
				);
					...
					<Button
						...
						onClick={async () => {
							const updateSolution =
								await SolutionService.postSolutionChangeScore(
									current_solution!.id,
									true
								);
							setSolution(updateSolution);
...
export const TaskInfo: (props: ITaskInfo) => JSX.Element = (
	props: ITaskInfo
) => {
	const theme = {
	...
	const { current_solution, isChanged } = useTypedSelector(
	...
	return (
		<Flex direction="column">
			...
			>
				<Stat>
					<StatLabel>
						<Text fontSize="xl">{theme.text}</Text>
					</StatLabel>
					<StatNumber>
						<PopoverForm
							points={props.points}
					...
					<StatHelpText>
						<Text fontSize="md">Отправлено {format_date.toLocaleString()}</Text>
					</StatHelpText>
				</Stat>
			</Box>
			<Flex
				...
				<Editor
					...
					onChange={(value) => {
						if (value) {
							...
							if (!isChanged) setIsChangedSolution(true);
						}
					}}
