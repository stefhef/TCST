...
export const SolutionInfo: (props: ISolutionInfo) => JSX.Element = (
	props: ISolutionInfo
) => {
	...
	return (
		<SimpleGrid m={1}>
			<LinkBox
				as={'article'}
				borderWidth={
					current_solution && props.id === current_solution.id ? 3 : 1
				}
				borderRadius={10}
			...
			>
				<HStack>
					...
					>
						...
						<Text fontSize="sm">
							{props.time_finish
								? get_format_date(props.time_finish)
								: get_format_date(props.time_start)}
						</Text>
					</VStack>
					<Spacer />
					<Text fontSize={'sm'}>
