...
export const TaskPreviewStudent: (props: ITaskPreviewStudent) => JSX.Element = (
	props: ITaskPreviewStudent
) => {
			...
			<BorderShadowBox padding="0.5vw" mb="5px">
				<VStack alignItems={'left'}>
					<HStack>
						...
						<SkeletonText isLoaded={Boolean(isLoaded)} noOfLines={1}>
							<Text align="right" color={status?.iconColor}>
								{`${status?.textStatus}`}
							</Text>
						</SkeletonText>
					</HStack>
					<Progress
						colorScheme={status ? status.progressColor : 'gray'}
						w={'100%'}
						size="lg"
						borderRadius="lg"
