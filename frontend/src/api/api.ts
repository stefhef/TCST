			...
			return await axios(axiosRefreshTokenRequestConfig)
				.then(async (refresh_token_response) => {
					...
					localStorage.setItem(
						'access_token',
						decodeLocal(login_data.access_token)
					);
...
export const baseURL =
	process.env.NODE_ENV === 'production'
		? process.env.REACT_APP_PROD_SITE_URL
		: process.env.REACT_APP_DEV_SITE_URL;
export const vkClientId =
	process.env.NODE_ENV === 'production'
		? process.env.REACT_APP_PROD_VK_CLIENT_ID
		: process.env.REACT_APP_DEV_VK_CLIENT_ID;
console.log(baseURL);
