...
export default class UserService {
	static async getStudentsWithSolution(
		group_id: number | string,
		course_id: number | string,
		task_id: number | string
	): Promise<IStudentWithSolution[]> {
	...
	static async getStudentsGroup(group_id: number | string): Promise<IUser[]> {
		const requestConfig: IRequestConfig = {
			method: 'get',
			url: '/user/students_group',
			auth: true,
			params: {
				group_id,
			},
